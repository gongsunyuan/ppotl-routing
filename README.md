# experiments — TRL routing ablation package (M0-1)
...
- `src/trl_sb3/` — package: `env/` `policy/` `train/` `eval/` `common/` `run/`
  (business modules land in M0-2+; `common/` holds config + logging primitives).
- `config/` — `default.yaml` (paper_defaults). Grid / prereg land in M2-4/M3.
...
& $py -m trl_sb3.run sweep --grid config/grid_main.yaml            # M2-4
& $py -m trl_sb3.run sweep --grid config/grid_main.yaml --dry-run  # M2-4
& $py -m pytest tests -q
```
